...
const CreateEspacioForm: React.FC<CreateEspacioForm> = ({ onUpdateStepErrors, onClose, isLoading, activeStep, handleNext, handlePrev, formSubmited, newEspacio, onFinalization }) => {
    ...
    useEffect(() => {
        onUpdateStepErrors({
            step0: !!((errors.NombreEspacio && touched.NombreEspacio) || (errors.DescripcionEspacio && touched.DescripcionEspacio) || (errors.CapacidadEspacio && touched.CapacidadEspacio)),
    ...
    return (
        <Form style={{ width: '100%' }} id="formCreateEspacio">
            <div className={activeStep === 0 ? `${styles.form_step} ${styles.show}` : `${styles.form_step} ${styles.hide}`}>
                ...
                <Field id="NombreEspacio" name='NombreEspacio' placeholder="Ingresa el nombre" className={errors.NombreEspacio && touched.NombreEspacio ? styles.error_input : ''} disabled={formSubmited} />
                <ErrorMessage name="NombreEspacio" component="div" className={styles.error} />

                <label htmlFor="DescripcionEspacio">Descripción</label>
                <Field as="textarea" id="DescripcionEspacio" name='DescripcionEspacio' placeholder="Ingresa una descripción para el espacio" className={errors.DescripcionEspacio && touched.DescripcionEspacio ? styles.error_input : ''} disabled={formSubmited} />
                <ErrorMessage name="DescripcionEspacio" component="div" className={styles.error} />

                <label htmlFor="CapacidadEspacio">Capacidad</label>
                <Field type="number" id="CapacidadEspacio" name='CapacidadEspacio' placeholder="Ingresa la capacidad" className={errors.CapacidadEspacio && touched.CapacidadEspacio ? styles.error_input : ''} disabled={formSubmited} />
                ...
                <Field as="textarea" id="DescripcionUbicacion" name='DescripcionUbicacion' placeholder="Escribe alguna ubicación referencial para el espacio" className={errors.DescripcionUbicacion && touched.DescripcionUbicacion ? styles.error_input : ''} disabled={formSubmited} />
            ...
            <div className={activeStep === 1 ? `${styles.form_step} ${styles.show}` : `${styles.form_step} ${styles.hide}`}>
                {isEspecificDataReady && !isEspecificDataError ? (
                    <>
                        ...
                        <Field name="CodTipoEspacio" id="CodTipoEspacio" disabled={formSubmited}>
                        ...
                        <Field name="CodUnidad" id="CodUnidad" disabled={formSubmited}>
                            {({ field, form }: { field: any, form: any }) => {
                                const handleChange = (selectedOption: any) => {
                                    if (selectedOption) {
                                        form.setFieldValue(field.name, selectedOption.value);
                        ...
                        <Field name="PersonasInternas" id="PersonasInternas" disabled={formSubmited}>
                            {({ field, form }: { field: any; form: any }) => {
                        ...
                        <Field type="number" id="DiasAntelacion" name='DiasAntelacion' placeholder="Ingrese cuántos días de antelación" className={errors.DiasAntelacion && touched.DiasAntelacion ? styles.error_input : ''} disabled={formSubmited} />
            ...
            <div className={styles.step_buttons}>
                ...
                <button type="button" onClick={handleNext} disabled={(activeStep === 1 && !formSubmited) || activeStep === 3}>
                ...
                activeStep === 1 && (
                    <div>
                        {isLoading ? (
                            ...
                            <div className={styles.card_buttons_container}>
                                <button type="submit" disabled={isSubmitting || formSubmited}>
...
export const CreateEspacioModal: React.FC<CreateEspacioModal> = ({
    isOpen,
    onClose,
}) => {
    ...
    const createEspacio = async (values: any, { setSubmitting, resetForm }: { setSubmitting: (isSubmitting: boolean) => void; resetForm: () => void; }) => {
        setSubmitting(true);
        setIsLoading(true);

        try {
            const response = await axios.post(`${API_BASE_URL}/api/espacios`, {
            ...
            setNewEspacio(newEspacio);
            Store.addNotification({
                title: "Espacio creado con exito",
                message: "El espacio " + response.data.NombreEspacio + " se ha creado con éxito, ahora puede agregar las fotos, restricciones y equipos del espacio si lo desea.",
                type: "success",
                insert: "top",
                container: "top-left",
                animationIn: ["animate__animated", "animate__fadeIn"],
            ...
            /*router.push(`${API_BASE_URL}/espacios/${newEspacio.CodEspacio}`);*/
            setFormSubmited(true);
        } catch (error) {
            Store.addNotification({
                title: "Ha ocurrido un problema al crear el espacio",
                message: "Lo sentimos ha ocurido un problema al crear el espacio, vuelva a intentarlo mas tarde",
                type: "danger",
                insert: "top",
                container: "top-left",
                animationIn: ["animate__animated", "animate__fadeIn"],
    ...
    const finalizationModal = () => {
        closeModal();
        if (newEspacio.CodEspacio !== undefined) {
            window.open(`/espacios/${newEspacio.CodEspacio}`, '_blank');
        }
    }
