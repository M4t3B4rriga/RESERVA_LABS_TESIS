...
VALUES (3, 12, NULL, NULL, NULL, NOW(), 1)
...
SELECT * FROM T_MSITEM_MENU WHERE IME_NOMBRE = 'Solicitudes de Registro';

INSERT INTO T_MSPERMISO (PK_TMSROL, PK_TMS_ITEM_MENU, PK_TMSSUBITEM_MENU, PK_TMSSUB_SUBITEM_MENU, PK_TMSFUNCIONALIDAD, PER_FECHA_ASIGNACION, ESTADO)
VALUES (3, (SELECT PK_TMS_ITEM_MENU FROM T_MSITEM_MENU WHERE IME_NOMBRE = 'Solicitudes de Registro'), NULL, NULL, NULL, NOW(), 1);
...
SELECT * FROM T_MCPERSONA_INTERNA;
SELECT * FROM T_REGISTRO_SOLICITUD;
SELECT * FROM T_MSAUDITORIA;
SELECT * FROM T_MSUSUARIO;
...
SELECT * FROM T_MSROL WHERE ESTADO = 1;
...
-- CREAR TABLA PARA CAMBIO DE CONTRASEÑA
-- Crear tabla para solicitudes de cambio de contraseña
CREATE TABLE T_CAMBIO_PASSWORD_SOLICITUD (
    PK_CAMBIO_PASSWORD_SOLICITUD INT AUTO_INCREMENT PRIMARY KEY,
    XEUSU_CODIGO INT NOT NULL,
    USU_NOMBRE VARCHAR(30) NOT NULL,
    PEI_EMAIL_INSTITUCIONAL VARCHAR(100) NOT NULL,
    PEI_NOMBRE VARCHAR(50) NOT NULL,
    ...
    ADMIN_PROCESADOR VARCHAR(100) NULL,
    CONSTRAINT FK_CAMBIO_PASSWORD_USUARIO 
        FOREIGN KEY (XEUSU_CODIGO) REFERENCES T_MSUSUARIO(XEUSU_CODIGO) 
        ON DELETE RESTRICT ON UPDATE RESTRICT
) COMMENT 'Tabla para gestionar solicitudes de cambio de contraseña';

-- Crear índices para mejorar el rendimiento
CREATE INDEX IDX_CAMBIO_PASSWORD_ESTADO ON T_CAMBIO_PASSWORD_SOLICITUD(ESTADO);
CREATE INDEX IDX_CAMBIO_PASSWORD_FECHA ON T_CAMBIO_PASSWORD_SOLICITUD(FECHA_SOLICITUD);
CREATE INDEX IDX_CAMBIO_PASSWORD_USUARIO ON T_CAMBIO_PASSWORD_SOLICITUD(XEUSU_CODIGO);

-- =====================================================================
-- AGREGAR ÍTEM DE MENÚ PARA GESTIÓN DE CAMBIOS DE CONTRASEÑA
-- =====================================================================

-- Insertar nuevo ítem de menú para gestionar cambios de contraseña
INSERT INTO T_MSITEM_MENU (IME_NOMBRE, IME_URL, IME_POSICION, IME_ICONO, ESTADO)
VALUES ('Cambios de Contraseña', '/manage-password-changes', 14, 'fas fa-key', 1);
